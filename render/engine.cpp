...
void EngineQt::render(QPainter &p) {
    ...
    const QVector<QVector3D> &vColorMF = model->getColorF();
    const QVector<const QColor *> &vColorMQ = model->getColorQ();
    const QVector<QVector3D> &vPositionM = model->getPosition();
    const QVector<GLuint> &vIndexMT = model->getIndexT(dir);

    // draw model lines
    QPoint pnt;
    int cnt = 0;
    const QVector<GLuint> &vIndexML = model->getIndexL(dir);
    for (auto i : vIndexML) {
        if (cnt++ == 1) {
            p.setPen(*vColorMQ[i]);
            ...
            p.drawLine(pnt, tmp);
            cnt = 0;
        } else pnt = (matFinM * vPointM[i]).toPoint();
    ...
    for (auto i : vIndexMT) {
        ...
        if (cnt++ == 2) {
            ...
            float spec = pow(std::max(dot(reflekt, -lightIn), 0.0f), 8);
            float base = 0.55 + std::max(0.3f * dot(normal, -lightIn), 0.0f);
